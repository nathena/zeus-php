...
class PDOp
{
    ...
    public function openXa()
    {
        if( !$this->xid )
        {
            throw new ZeusException("The command cannot be executed when xa start,not xid");
        }

        $this->pdo->exec("XA START ".$this->xid);
    }
    ...
    public function prepareXa()
    {
        if( !$this->xid )
        {
            throw new ZeusException("The command cannot be executed when xa prepare,not xid");
        }
    ...
    public function commitXa()
    {
        if( !$this->xid )
        {
            throw new ZeusException("The command cannot be executed when xa commit,not xid");
        }

        $this->pdo->exec("XA COMMIT ".$this->xid);
    ...
    public function rollbackXa()
    {
        if( !$this->xid )
        {
            throw new ZeusException("The command cannot be executed when xa rollback,not xid");
        }

        $this->pdo->exec("XA ROLLBACK ".$this->xid);

        $this->xid = null;
    }

    public function xa($xid)
    {
        $this->xid = $xid;
    }
}